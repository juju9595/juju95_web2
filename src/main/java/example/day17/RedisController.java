...
public class RedisController {
    ...
    //{ "sno" : "2 " , "kor" : "100" , "math" : "100" }
    public ResponseEntity<?> update( @RequestBody example.day15.StudentDto studentDto ){
    ...
    // TTL : 레디스에 저장된 멘트리(key-value) 을 특정한 기간(시간)이 되면 자동 삭제
    @GetMapping("/auth/send")// http://localhost:8080/redis/auth/send?phone=01011111111
    public ResponseEntity<?> authSend(@RequestParam String phone){
        ...
        redisTemplate.opsForValue().set(key, code, Duration.ofSeconds(20)); // 10초
    ...
    public ResponseEntity<?> authConfirm(@RequestParam String phone, @RequestParam String code){
        String key = "auth:" +phone;// 1. 조회할 key 구성
