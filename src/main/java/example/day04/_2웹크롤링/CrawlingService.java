...
public class CrawlingService {
    ...
    public List<String> task1() {
        ...
        try{
            ...
            // JS : document.querySelect(".title > a");
            //JSOUP : document.selec(".title > a");
    ...
    public List<Map<String, String >> task2(){
        ...
        try{
            for (int page = 1; page <= 3; page++){//page를 1부터 3까지 반복
                //2-2 웹크롤링할 주소, 페이징처리 크롤링 //1페이지 대신에 변수를 넣어서 여번 크롤링
                ...
                Document document = Jsoup.connect(URL).get();
                //2-4 : 책제목(.info_name > .gd_name ) 과 책가격의 css 선택자 확인
                ...
                // 2-5 : 반복문을 이용한 책정보 구성
                for( int i = 0; i < nameList.size(); i++ ){//page를 1부터 3가지 반복
                    String name = nameList.get(i).text(); // i번째 책제목 1개씩 호출
                    String price = priceList.get(i).text(); // i번째 책가격 1개씩 호출
                    String img = imgList.get(i).attr("data-original"); //i번째 책 이미지 속성값 1개씩 호출
    ...
    public Map<String,String> task3(){
        Map<String, String> map = new HashMap<>(); //3-1 날시 정보를 저장할 앱
        try{
            String URL = "https://weather.daum.net/";//3-2 날씨 정보를 가져올(크롤링) 주소
            Document document = Jsoup.connect(URL).get();//3-3 JSOUP 활용한 지정한 주소 HTML 가져오기
            System.out.println("document = " + document);

            Element elements = document.selectFirst(".info_weather .num_deg");//3-4 온도(.info_weather .num_deg)
            //즉] : '.info_weathe' 마크업은 JS가 데이터를 표시하는 방법이므로 크롤링 불가능
