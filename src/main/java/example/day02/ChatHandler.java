...
@Component // 스프링 컨테이너(메모리)에 빈(객체) 등록, 싱글톤 대신해서..
public class ChatHandler extends TextWebSocketHandler {
    ...
    public void afterConnectionEstablished(WebSocketSession session) throws Exception {
        ...
        // 로그인 세션x WebSocketSession 이란 : ws 기반으로 클라이언트가 요청한 정보가 저장된 객체
        // HttpSession 이란 : http 기반으로 클라이언트가 요청한 정보가 저장된 객체
        System.out.println("[클라이언트 웹소켓 객체]" + session);
        // 1. 접속된 클라이언트 소켓들을 저장: 받은 메시지를 접속된 소켓들에게 재전송 하기 위해
    ...
    public void afterConnectionClosed(WebSocketSession session, CloseStatus status) throws Exception {
        ...
        // 1. 클라이언트 소켓과 연결이 끊겼을때 접속 명단에서 제외
        접속명단.remove(session);
    }
    ...
    protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
        System.out.println("[서버] 클라언트로부터 메시지가 도착");
