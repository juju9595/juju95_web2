...
public class UserController {
    ...
    //2-1. 로그인(+세션 : 자바웹서버(톰켓)의 임시 저장소, 한번 로그인 성공했다는 증거)
//    @PostMapping("/login")
...
//    }
    ...
    public ResponseEntity<?> login(@RequestBody UserDto userDto, HttpServletResponse response){
        UserDto result = userService.login(userDto);
        if( result != null){//만약에 로그인 성공했다면 성공한 유저의 아이디를 쿠키에 저장
            // *로그인정보를 세션에 저장하면 서버이므로 안전하다. 쿠키(클라이언트) 저장하면 위험하다.
            //Cookie cookie = new Cookie( "쿠키명", 값); // 주로 사용자들의 설정값
            // response.addCookie( 생성한 쿠키 );
            Cookie cookie = new Cookie("loginUser", result.getUid());
            // 클라이언트에서 해당 쿠키를 노출(탈취) 방지 = 주로 민감함정보는 httpOnly 설정한다.
            cookie.setHttpOnly( true ); // .setHttpOnly( true ) : 무조건 http 에서만 사용, 즉] JS로 접근 불가능 (민감한 정보일때 사용)
            cookie.setSecure( false ); // HTTP 암호화 :   단 https(true) 에서만 가능 하므로 테스트 단계에서는 false 한다. (민감한 정보일때 사용)
            // + 설정
