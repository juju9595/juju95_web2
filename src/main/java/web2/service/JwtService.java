...
public class JwtService {
    ...
    // 특정한 데이터를 직접적으로 보여주지 않고 징표를 대신 보여주는
    // 웹에서 데이터 숨기기(보안)

    // JMT 토큰 가능하지만, 데이터들 간의 서로다른 토큰 과 복잡한 게산식 추가
    // SHA-256 알고리즘 사용하여 비밀키(임의,개발자만들기, 토큰 풀기 위한 비밀번호)를 32글자이상
    // 1-2 : 내가 만든 임의의 토큰에 사용되는 계산식 비밀키
    private String 비밀번호 ="123456789123456789123456789123456789"; // 난수 키보다는 직접생성한 (임의)키 권장
    ...
    public String 토큰생성( String 데이터 ){
        ...
        String token = Jwts.builder() // .builder() : 빌더패턴(생성자 대신에 함수형 객체생성) 이용한 토큰 생성
                ...
                .setExpiration( new Date( System.currentTimeMillis()+1000 * 60 ) ) // 30초
    ...
    public boolean 토큰검증( String 토큰) {
        try {
            Jwts.parser() // parser
                    .setSigningKey(비밀키) // 서명 검증에 필ㅇ한 비밀키 대입
                    .build() // 비밀키 확인
                    .parseClaimsJws(토큰); // 검증할 토큰 대입
            return true; // 예외가 발생하지 않으면 토큰 생성
        } catch (Exception e) {
            return false; // 토큰 비정상
        }
    }
    ...
    public String 값추출(String 토큰){
        try{
            Claims claims = Jwts.parser() // parser
                    .setSigningKey(비밀키) // 서명 검증에 필ㅇ한 비밀키 대입
                    .build() // 비밀키 확인
                    .parseClaimsJws(토큰) // 검증할 토큰 대입
