...
public class TodoService {
    ...
    public TodoDto update(TodoDto todoDto){
        ...
        if(optional.isPresent()){
            ...
            return entity.todoDto(); // 수정된 엔티티를 dto로 반환
        }return null;
    }
    ...
    public Page<TodoDto> page(int page, int size){
        ...
        // PageRequest.of( 조회할페이지번호, 조회할페이지데이터수, Sort.by( Sort.Direction.DESC, "정렬속성명"))
        ...
        return result.map(TodoEntity::todoDto);
    }
    ...
    public Page< TodoDto > page2( String keyword, int page, int size){
        ...
        //5-1 : 만약에 검색이 없으면 전체 조회
        if( keyword == null || keyword.isBlank()){// 5-1 : 만약에 검색이 없으면 전체 조회
