...
public class BoardEntity {
    ...
    //** 단방향 연결 **//
    // 하위 엔티티가 상위 엔티티 참조관계
    @ManyToOne( cascade = CascadeType.ALL, fetch = FetchType.EAGER) // M:1 , 다수가 하나에게
    @JoinColumn( name = "cno")// PK 필드명(+PK 필드명 권장)
...
/*
    ...
    [ fetch ] 부모[PK] 와 자식[FK] 관계에서 엔티티를 조회하는 방법
        fetch = FetchType.EAGER : 해당 엔티티를 조회하면 참조 엔티티도 즉시 조회한다.
            - 특징 : 기본값, 초기 로딩 느리다, 불필요한 엔티티 정보가 있을 경우 기능 저하
        ...
        * 동일한 SELECT(조회) 된 결과는 MYBATIS와 JPA 는 자동 1차 캐싱(기록)
