...
const nickName = `익명${randomId}` // 익명{난수} // 회원제 일 경우 서버 로그인세션정보
...
const client = new WebSocket("/chat"); // 자바의 WebSocketConfig에서 정의한 주소 확인

// [2] 서버 웹 소켓과 연동 성공했을때, 
// event란? 함수의 매개변수 이면서 *해당 이벤트 정보*를 담고 있는 객체
client.onopen = (event) => {
    console.log("======서버 소켓과 연동 성공했다.======")
    //(1). ======== 방번호에 특정한 닉네임 **접속** 메시지 ========
    let msg = {type: "join", room : room , nickName : nickName}//JSON 형식으로 문자 메시지 보내기
    //JSON.stringify()  : 객체(JSON)타입을 형식을 유지하고 문자열 타입으로 변환
...
client.onmessage = (event) => {
    ...
    console.log(event.data); // 4-1 서버로부터 받은 메시지(data 속성) 확인
    ...
    }else if( message.type == 'msg'){//***내가 보낸 메시지***
    ...
    // 4-4 구성한 html를 <div class = "msgbox"> 에 추가하기
...
const onMsgSend = () => {
    ...
    if(message == '') return //5-2. 만약에 값이 없으면 종료
    //5-3. 메시지 구성하기
    ...
    //5-4 구성한 메시지를 클라이언트소켓이 서버에게 보내기
    client.send(JSON.stringify(msg))
    //5-5 input 마크업 초기화
    msginput.value = '';
} //func e
...
const enterKey = () =>{
    //만약에 input에서 enter(13)키를 눌렀을때, keyCode에서는 enter가 13 이다.
