...
export default function(props){
    ...
    const arrayAdd = () =>{// 주의할점은 useState는 수박을 관리하는게 아니라 수박을 포함하는 [] 관리
...
/*
    1. 데이터/자료 : 값
    2. 자료타입 : 갑의 분류(한국 분리수거 VS 미국 분리수거)
        - 기본타입(리터럴(공유/static):미리만들어진데이터들) VS 참조타입(객체/주소값)
    3. 자료의 주소값 변경 기준 
        1 -> 2 : 리터럴/(주소) 변경
        'a' -> 'b' : 주소 변경
        {name : 유재석} -> {name : 유재석 , age : 40} : 주소변경 x 
        [ '수박' ] -> ['수박','사과'] : [] 주소변경 x
