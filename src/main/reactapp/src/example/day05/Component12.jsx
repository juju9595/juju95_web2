...
function Page404( props ){
    ...
    return(<>
        ...
        <a href="/">홈으로1(get방식)</a> <br/>
        <Link to="/">홈으로2(react라우터방식)</Link>
        <button onClick={이동함수}> 흠으로3 </button>
    </>)
}


//[*] 라우터 : 하나의 컴포넌트가 여러 컴포넌트를 연결 구조 // 가상의 URL 만들기
//1. 라우터 라이브러리 설치 : npm i react-router-dom, *리액트 종료된 상태에서 진행, 설치후 리액트 실행
//2. 
export default function Component12(props){
    return(<>
    <BrowserRouter>
        <ul>
            <a href = "/"> 메인페이지(home/html방식) </a> <br/>
            {/* 진짜 URL 이 아닌 가상의 URL 로 이동할때는 <a> 마크업 대신 <Link to ="가상URL"> 사용 */}
            <Link to="/"> 메인페이지(home/react라우터방식)</Link> <br/>
            <Link to="/about"> 소개페이지 </Link> <br/>
            <Link to="/mypage"> 마이페이지1(쿼리스트링x) </Link> <br/>
            <Link to="/mypage?name=유재석&age=40"> 마이페이지2(쿼리스트링o) </Link> <br/>
            <Link to="/product"> 제품소개페이지1(path x)</Link> <br/>
            <Link to="/product/코카콜라/1000"> 제품소개페이지2(path o)</Link>
        </ul>
        <Routes>{/* 가상의 URL 정의하고 정의한 URL과 메핑할 컴포넌트 정의 */}
            <Route path='/' element = {<Home/>}/>           {/*Route path="가상URL정의" element = {<컴포넌트/>} */}
            <Route path='/about' element ={<About/>}/>
            <Route path='/mypage' element = {<MyPage/>}/>
            <Route path='/product/:name/:price' element = {<Product/>}/>
            {/*만약에 존재하지 않은 가상 URL 요청하면 404 컴포넌트 */}
