...
export default function Component11(props){
    ...
    //[1] 렌더링을 하지 않고 데이터를 참조하는 훅 , useRef vs useState
    const inputRef = useRef(null); //1. import { useRef }form "react" 또는 자동완성
                                   // 2. useRef(초기값)
    const 등록함수 = () =>{ 
        console.log(inputRef);          // 3. inputRef :  현재 참조중인 객체 정보 { current : 참조값 }
        console.log(inputRef.current);  // 4. useRef.currnet : 참조값, <input>
